...
void dfs(int node , int parent , int level){
    ...
    for(int next : v[node]){
        if(next != parent){
            ...
            if(far1[next] + 1 > far1[node]){
                far3[node] = far1[node];
                far1[node] = far1[next] + 1;
            }
            else if(far1[next] + 1 > far3[node]){
                far3[node] = far1[next] + 1;
            }
        }
    }
    far2[node] = 0;
...
int val(int root , int node){
    if(isanc(node , root)){
        int ret = far2[node];
        int nx = moveup(root , depth[root] - depth[node] - 1);
        if(far1[node] == 1 + far1[nx]){
            ret = max(ret , far3[node]);
        }
        else{
            ret = max(ret , far1[node]);
        }
        return ret;
    }
    else{
